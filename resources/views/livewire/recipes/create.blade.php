<div class="mb-4" x-data="{ isOpen: {} }" x-on:recipe-created="$refs.modal.hidePopover()">
    <button type="button" x-on:click="$refs.modal.showPopover()"
        class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded shadow-md transition duration-300">
        <i class="fas fa-plus mr-2"></i>Ajouter une nouvelle recette
    </button>
        ...
        <form class="space-y-6" wire:submit.prevent="create">
            ...
            <div>
                ...
                <input type="text" id="title" value="{{ old('title') }}"
                    ...
                    placeholder="Ex: Gâteau au chocolat" wire:model.defer="title">
                @error('title')
            ...
            <div>
                ...
                <textarea id="description" cols="30" rows="4" value="{{ old('description') }}"
                    ...
                    placeholder="Description de la recette" wire:model.defer="description"></textarea>
                @error('description')
                    <div class="mt-1 text-sm text-red-600 bg-red-100 p-2 rounded-md">
                        {{ $message }}
                    </div>
